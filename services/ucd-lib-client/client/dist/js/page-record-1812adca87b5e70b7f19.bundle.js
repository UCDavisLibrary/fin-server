...
`}class S extends k.a{static get properties(){return{itemHeight:{type:Number,attribute:"item-height"},items:{type:Array},renderedItems:{type:Array}}}constructor(){super(),this.render=$.bind(this),this.itemHeight=20,this.renderedItems=[],this.items=[],this.height=-1,this._cacheHeight=this._cacheHeight.bind(this),this.addEventListener("scroll",()=>this._onViewportUpdate())}firstUpdated(){this.positionEle=this.querySelector(".app-virtual-scroller-scroll-panel")}connectedCallback(){super.connectedCallback(),window.addEventListener("resize",this._cacheHeight),this._cacheHeight()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("resize",this._cacheHeight)}createRenderRoot(){return this}_cacheHeight(e=!0){this.height=this.offsetHeight,!0===e&&this._onViewportUpdate()}setItemRenderer(e,t){this.renderItem=e,this.renderItemScope=t||this}updated(e){(e.has("itemHeight")||e.has("items"))&&this._onViewportUpdate(!0),e.has("items")&&(this.positionEle.style.height=this.itemHeight*(this.items.length-1)+"px")}_onViewportUpdate(e=!1){this.height<=0&&this._cacheHeight(!1);let t=Math.floor(this.scrollTop/this.itemHeight)-1;t<0&&(t=0);let i=t+Math.ceil(this.height/this.itemHeight)+1;if(i>=this.items.length&&(i=this.items.length),this.firstItem===t&&this.lastItem===i&&!1===e)return;this.firstItem=t,this.lastItem=i;let n=[];for(let e=t;e<i;e++)n.push({index:e,top:this.itemHeight*e});this.renderedItems=n}renderItems(){return this.renderedItems.length>this.items.length?k.b``:this.renderedItems.map(e=>k.b`
      <div style="position: absolute; left: 0; right: 0; top: ${e.top}px; height: ${this.itemHeight}px">
        ${this.renderItem.apply(this.renderItemScope,[e.index])}
      </div>
    `)}renderItem(e){throw new Error("You must override this method")}}customElements.define("app-virtual-scroller",S);i(181),i(79);const T={folder:["folder"],"fin-icons:image-solid":["tif","tiff","gif","jpg","jp2","jpeg","webp","bmp","png"],"fin-icons:video-solid":["avi","mp4","flv","wmv","mov"],"fin-icons:sound-solid":["wav","mp3","mid","aif"],"fin-icons:text-solid":["doc","docx","txt","rtf",".odt"],"fin-icons:spreadsheet-solid":["ods","csv","tsv","xsl","xslx"],"fin-icons:pdf-solid":["pdf"],"fin-icons:compressed-solid":["zip","rar","arj","gz","tgz"]},I="fin-icons:file-solid";class R extends(Mixin(k.a).with(LitCorkUtils)){static get properties(){return{title:{type:String},loadingFiles:{type:Boolean},loadingSearch:{type:Boolean},currentDir:{type:String},files:{type:Array},selectedFile:{type:String},mode:{type:String},thumbnail:{type:String},lineHeight:{type:Number}}}constructor(){super(),this.render=M.bind(this),this.reset(),this._injectModel("AppStateModel","RecordModel"),this.iconMap={};for(let e in T)for(let t of T[e])this.iconMap[t]=e;window.addEventListener("resize",()=>this._onResize())}firstUpdated(){this.contentBody=this.shadowRoot.querySelector(".content-body"),this.scrollPanel=this.shadowRoot.querySelector("app-virtual-scroller"),this.scrollPanel.setItemRenderer(this.renderRow,this),this.parentNode.removeChild(this),document.body.appendChild(this),this.filenameWidth="30px"}updated(e){if(e.has("selectedFile")){for(let e of this.files)e.selected=e.fullUrl===this.selectedFile;this.scrollPanel.requestUpdate()}}_onResize(){if(!this.contentBody)return;window.innerWidth>700?this.scrollPanel.style.height=window.innerHeight-335-100+"px":this.scrollPanel.style.height=window.innerHeight-335+"px",this.filenameWidth=this.scrollPanel.offsetWidth-155+"px",this.scrollPanel.requestUpdate()}_onAppStateUpdate(e){if(!e.selectedRecord)return this.reset();this.selectedRecord!==e.selectedRecord&&(this.reset(),this.selectedRecord=e.selectedRecord,this.selectedRecordMedia=e.selectedRecordMedia,this.files=[],this.currentDir="",this.selectedRecord&&this.selectedRecord["@type"].includes("http://digital.ucdavis.edu/schema#bagOfFiles")&&(this._browseDirectory(),this.title=this.selectedRecord.name||this.selectedRecord.title,this.thumbnail=this.selectedRecord.thumbnailUrl||""))}async show(){this.style.display="block",document.body.style.overflow="hidden",window.scrollTo(0,0),this._onResize(),setTimeout(()=>{this._onResize()},50),this._onAppStateUpdate(await this.AppStateModel.get())}hide(){this.style.display="none",document.body.style.overflow="auto"}reset(){this.loadingFiles=!1,this.loadingSearch=!1,this.currentDir="/",this.files=[],this.lineHeight=41,this.selectedFile=""}_renderBreadcrumbs(){if("search"===this.mode)return k.b`<iron-icon icon="chevron-right"></iron-icon>
