...
class RecordsModel extends ElasticSearchModel {
   ...
   * @param {Boolean} seo
   */
  async _fillGraphRecord(graph, record, seo, crawled={}) {
    crawled[record['@id']] = true;

    for( var i = 0; i < FILL_ATTRIBUTES.length; i++ ) {
      if( !record[FILL_ATTRIBUTES[i]] ) continue;
      await this._fillGraphAttribute(graph, record, FILL_ATTRIBUTES[i], seo, crawled);
    }
  }
   ...
   * @param {Boolean} seo
   */
  async _fillGraphAttribute(graph, record, attribute, seo, crawled={}) {
    ...
    for( var i = 0; i < newDocs.length; i++ ) {
      ...
      await this._fillGraphRecord(graph, childRecord, seo, crawled);
    }
  }
